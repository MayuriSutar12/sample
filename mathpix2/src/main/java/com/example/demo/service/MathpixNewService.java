...
//    public ApiResponse createTempListForConversion(QuestionTempListDto questionTempListDto) {
//        if (questionTempListDto == null || questionTempListDto.getQuestions() == null) {
//            return new ApiResponse(HttpStatus.BAD_REQUEST, "Invalid input, questionId is missing", null);
//        }
//
//        //  Check if tables already contain data, return conflict if true
////        if (questionDataTempRepository.count() > 0 ||
////                optionsDataTempRepository.count() > 0 ||
////                solutionDataTempRepository.count() > 0) {
////            return new ApiResponse(HttpStatus.BAD_REQUEST, "Data already exists in the tables. Clear existing data before adding new entries.", null);
////        }
//
//        List<Map<String, Object>> extractedDataList = new ArrayList<>();
//
//        for (QuestionTempSaveDto question : questionTempListDto.getQuestions()) {
//            Long questionId = question.getQuestionId();
//            // String questionImageUrl = question.getQuestionImageUrl();
//
//            log.info("Processing Question ID: {}", questionId);
//
//            //  Fetch options related to this questionId
//            List<QuestionWithOptionsProjection> results = optionsDataRepository.findOptionsByQuestionId(questionId);
//
//            if (results.isEmpty()) {
//                log.warn("No options found for question ID: {}", questionId);
//                continue; // Skip to the next question if no options found
//            }
//
//            //  Extract question details from the first row
//            QuestionWithOptionsProjection firstRow = results.get(0);
//            Integer questionType = firstRow.getQuestionType();
//            Integer questionSubType = firstRow.getQuestionSubType();
//            String solutionImage = firstRow.getSolutionImageUrl();
//            String questionImage = firstRow.getQuestionImageUrl();
//
//            String questionImageUrl = imgBaseUrl.concat(questionImage);
//            String solutionImageUrl = imgBaseUrl.concat(solutionImage);
//
//            log.info("Question Image URL: {}", questionImageUrl);
//
//            log.info("Solution Image URL: {}", solutionImageUrl);
//
//            //  Ensure question data is inserted before solution data
//            QuestionDataTemp questionDataTemp = QuestionDataTemp.builder()
//                    .questionId(questionId)
//                    .questionImageUrl(questionImageUrl)
//                    .questionType(questionType)
//                    .questionSubType(questionSubType)
//                    .createdOn(LocalDateTime.now())
//                    .genAiConversionCompleted(false)
//                    .build();
//
//            questionDataTemp = questionDataTempRepository.save(questionDataTemp); // Save the question first
//            log.info("Saved question data for question ID: {}", questionId);
//
//            //  Extract options and save them **only if they don't already exist**
//            if (!optionsDataTempRepository.existsByQuestion(questionDataTemp)) {
//                List<OptionsDataTemp> optionsDataTempList = new ArrayList<>();
//                for (QuestionWithOptionsProjection row : results) {
//                    OptionsDataTemp optionsDataTemp = OptionsDataTemp.builder()
//                            .optionId(row.getOptionId())
//                            .isAnswer(row.getIsAnswer())
//                            .answer(row.getAnswer())
//                            .question(questionDataTemp) // Link option to its question
//                            .build();
//                    optionsDataTempList.add(optionsDataTemp);
//                }
//
//                optionsDataTempRepository.saveAll(optionsDataTempList);
//                log.info("Saved options data for question ID: {}", questionId);
//            } else {
//                log.warn("Options data already exists for question ID: {}", questionId);
//            }
//
//            // Ensure solution data is inserted **only if it doesn't already exist**
//            try {
//                if (!solutionDataTempRepository.existsByQuestion(questionDataTemp)) {
//                    SolutionDataTemp solutionDataTemp = SolutionDataTemp.builder()
//                            .question(questionDataTemp)
//                            .solutionImageUrl(solutionImageUrl)
//                            .build();
//
//                    solutionDataTempRepository.save(solutionDataTemp);
//                    log.info("Saved solution data for question ID: {}", questionId);
//                    QuestionInfoAI questionInfoAI = QuestionInfoAI.builder()
//                            .questionId(questionId)
//                            .createdOn(LocalDateTime.now())
//                            .build();
//                    questionInfoAiRepository.save(questionInfoAI);
//                } else {
//                    log.warn("Solution data already exists for question ID: {}", questionId);
//                }
//
//            } catch (DataIntegrityViolationException e) {
//                log.error("Duplicate entry detected for question ID: {} - Solution data already exists.", questionId, e);
//            }
//        }
//
//        return new ApiResponse(HttpStatus.OK, "Data extracted and saved successfully", extractedDataList);
//    }
public ApiResponse createTempListForConversion(QuestionTempListDto questionTempListDto) {
    if (questionTempListDto == null || questionTempListDto.getQuestions() == null) {
        return new ApiResponse(HttpStatus.BAD_REQUEST, "Invalid input, questionId is missing", null);
...
//        }
    ...
    for (QuestionTempSaveDto question : questionTempListDto.getQuestions()) {
        ...
        String questionImageUrl =encodeImageUrl(imgBaseUrl.concat(questionImage));
        String solutionImageUrl = encodeImageUrl(imgBaseUrl.concat(solutionImage));
        ...
        questionDataTemp = questionDataTempRepository.save(questionDataTemp);
        log.info("Saved question data for question ID: {}", questionId);
        ...
        try {
            if (!solutionDataTempRepository.existsByQuestion(questionDataTemp)) {
                ...
                // Check for duplicates before inserting into M_questioninfo_ai
                if (!questionInfoAiRepository.existsByQuestionId(questionId)) {
                    QuestionInfoAI questionInfoAI = QuestionInfoAI.builder()
                            .questionId(questionId)
                            .createdOn(LocalDateTime.now())
                            .build();
                    questionInfoAiRepository.save(questionInfoAI);
                    log.info("Saved QuestionInfoAI for question ID: {}", questionId);
                } else {
                    log.warn("QuestionInfoAI already exists for question ID: {}", questionId);
                }
    ...
    public static String encodeImageUrl(String url) {
        ...
        // Define the base URL part that should remain unchanged
        String baseUrl = "https://jeeni-question-images.s3.ap-south-1.amazonaws.com/question-images/";

        if (!url.startsWith(baseUrl)) {
            return url; // If URL doesn't match expected format, return as-is
        }

        // Extract the file path after "question-images/"
        String filePath = url.substring(baseUrl.length());

        try {
            // Encode only the extracted file name
            String encodedFilePath = URLEncoder.encode(filePath, StandardCharsets.UTF_8.toString())
                    .replace("+", "%2B")   // Encode `+` as `%2B`
                    .replace("=", "%3D")   // Encode `=` as `%3D`
                    .replace("%2F", "/");  // Keep `/` unchanged

            return baseUrl + encodedFilePath; // Return the fully constructed URL
        } catch (Exception e) {
            System.err.println("Error encoding URL: " + url);
            return url; // Return original URL if encoding fails
        }
    }
    public ApiResponse processTempList() {
        List<QuestionDataTemp> questionDataTemp = questionDataTempRepository.findAll();
        StringBuilder errorMessages = new StringBuilder();
        if (questionDataTemp.isEmpty()) {
            return new ApiResponse(HttpStatus.NOT_FOUND,"No questions found to process.", null);
        }
        for (QuestionDataTemp question : questionDataTemp) {
            Long questionId = question.getQuestionId();
...
//                optionsDataTemp.setOptionLatex(questionJsonDTO.getOptionsLatex().toString());
//                optionsDataTemp.setGenAiAnswer(answerKey);
//                // optionsDataTemp.setGenAiIsAnswer(true);
//            }
//
